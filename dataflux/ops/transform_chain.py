...
one node in FluxStudio (dynamic ``op_0``, ``op_1``, … ``DATAFLUX_OP``
inputs instead of N wired ``DATAFLUX_SAMPLE`` connections) and one named
...
class TransformChain:
    ...
    in FluxStudio (dynamic ``op_0``, ``op_1``, … ``DATAFLUX_OP`` inputs)
    ...
    :class:`~dataflux.ops.random_apply.RandomApply` inside the chain to
    gate individual ops stochastically.

    YAML example::

        - !class:dataflux.ops.transform_chain.TransformChain
          ops:
            - !class:dataflux.ops.random_apply.RandomApply
              op: !class:waivefront.torchsig.processing.AWGNOp {}
              probability: 0.8
            - !class:dataflux.ops.torch.ToTensorOp {}

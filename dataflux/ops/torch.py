...
class FourierOp:
    ...
    **Windowing & units.** Mirrors the numpy ``FourierOp``: ``window`` applies a
    :func:`dataflux.windows.get_window` taper before the transform (default ``"boxcar"`` = none) and
...
class WindowOp:
    ...
    The tensor counterpart of :class:`dataflux.ops.numpy.WindowOp`: multiplies the signal by a
    :func:`dataflux.windows.get_window` taper (broadcast along ``dim``) and stashes the window
...
class SpectrumScalingOp:
    ...
    The tensor counterpart of :class:`dataflux.ops.numpy.SpectrumScalingOp`: amplitude (V) / power

...
The *fittable* companion to the config-pinned :class:`~dataflux.ops.target.EncodeTargetOp` /
:class:`~dataflux.ops.target.DecodeTargetOp`. Those ops carry an explicit ``mapping`` that is
...
* :meth:`LabelMap.encode_op` / :meth:`LabelMap.decode_op` hand back the dataflux ops that apply it.
...
is imported lazily inside :meth:`fit` so importing dataflux never pulls it in.
"""
...
class LabelMap:
    ...
    def encode_op(self, ignore_unknown: bool = False, default: Any = 0) -> EncodeTargetOp:
        """Return an :class:`~dataflux.ops.target.EncodeTargetOp` that maps name → id via this map."""
    ...
    def decode_op(self, ignore_unknown: bool = False, default: Any = None) -> DecodeTargetOp:
        """Return a :class:`~dataflux.ops.target.DecodeTargetOp` that maps id → name via this map."""

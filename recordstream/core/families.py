...
from recordstream.items import Boxes, NDArrayItem, Record, with_data
...
#: Transform classes already warned about (see :func:`_warn_if_boxes_are_left_behind`). Keyed
...
def _warn_if_boxes_are_left_behind(record: Record, op: Any, passed: Dict[str, Any]) -> None:
    """Warn once when a geometry-changing transform ran while a ``Boxes`` sat out the call.

    This family passes the op EXACTLY the keys of albumentations' own vocabulary, which is what
    lets a bare library transform work unmodified — but a detection target rides as a ``Boxes``
    ...
    error because a record may legitimately carry boxes describing something other than the
    ...
    coupled step over the ``Boxes`` form directly.
    """
    ...
    logger.warning(
        ...
        f"boxes ({keys}) ride as a Boxes item, which is not in the library's key vocabulary "
...
def _invoke_albumentations(record: Record, op: Any) -> Optional[Record]:
    ...
    if not kwargs:
        ...
        return record
    _warn_if_boxes_are_left_behind(record, op, kwargs)
    out = op(**kwargs)
    merged = dict(record)
...
def _invoke_torchvision_v2(record: Record, op: Any) -> Optional[Record]:
    ...
    ``tv_tensors`` types, and a :class:`~recordstream.Boxes` is not one, so a geometric
    ...
    """
    _warn_if_v2_leaves_boxes_behind(record, op)
    return cast(Record, op(record))


def _warn_if_v2_leaves_boxes_behind(record: Record, op: Any) -> None:
    """The v2 twin of :func:`_warn_if_boxes_are_left_behind` — once per transform type."""
    ...
    logger.warning(
        ...
        f"({keys}) ride as a Boxes item, which is not one of v2's tv_tensors types — so v2 "

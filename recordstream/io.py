...
shapes (an :class:`~recordstream.items.NDArrayItem` subclass → the array + its declared
...
The registered TYPE NAME (via :func:`~recordstream.items.register_item` /
:func:`~recordstream.items.get_item_type`) is the on-disk type tag — decoding requires the
...
from recordstream.items import NDArrayItem, Record, get_item_type, is_item, item_data

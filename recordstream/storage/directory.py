...
#: Record-layout filenames inside each per-record directory.
...
class DirectorySink(Storage, DataSink):
    ...
    def _write_record(self, record: Record) -> None:
        ...
        storage edits.
        """
        record_dir = self.path / f"{self._counter:06d}"
        record_dir.mkdir(parents=True, exist_ok=True)

        spec: Dict[str, Any] = {"recordstream_format": TYPED_FORMAT, "fields": []}
        ...
        (record_dir / _FIELDS_JSON).write_text(json.dumps(spec, indent=2))
        if payloads:
            np.savez(record_dir / _FIELDS_NPZ, **payloads)
        self._counter += 1
...
class DirectorySource(Storage, DataSource):
    ...
    def __iter__(self) -> Iterator[Record]:
        for record_dir in self._record_dirs():
            yield self._read(record_dir)

    def __len__(self) -> int:
        return len(self._record_dirs())

    @staticmethod
    def _read(record_dir: Path) -> Record:
        spec = json.loads((record_dir / _FIELDS_JSON).read_text())
        require_record_format(spec.get("recordstream_format"), "DirectorySource")
        npz_path = record_dir / _FIELDS_NPZ

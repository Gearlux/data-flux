...
Two pieces (mirroring the ``recordstream.projection`` protocol-plus-fallback design):
...
from recordstream.io import PLAIN_TYPE, encode_item
from recordstream.items import Record
from recordstream.ops.formula import _FORMULA_NAMESPACE
from recordstream.storage.base import PLAIN_VALUE, require_record_format, restore_attrs

logger = get_logger("recordstream.storage.query")
...
def scan_hdf5_metadata(path: Any) -> Iterator[Tuple[str, Dict[str, Any]]]:
    ...
    with h5py.File(str(path), "r") as handle:
        require_record_format(handle.attrs.get("recordstream_format"), "scan_hdf5_metadata")
...
def scan_zarr_metadata(path: Any) -> Iterator[Tuple[str, Dict[str, Any]]]:
    ...
    require_record_format(root.attrs.get("recordstream_format"), "scan_zarr_metadata")

"""Per-record named-cell store — the graph data plane for graph-shaped pipelines.

A :class:`Context` holds named **cells** for exactly one record's trip through the op
list: branch snapshots (a cell holding a record dict), captured
``@output`` values, and per-record parameters. The context ops in
:mod:`recordstream.ops.context` (``Save`` / ``Use`` / ``Drop`` / ``Apply`` / ``Capture`` /
``Mix``) move data between the linear record stream and these cells, which is what lets
a plain sequential op list execute a fan-out/fan-in graph.

The context ops route graph data through these per-record Context CELLS and never touch
the record's own fields — each typed item still owns its own metadata inside the record.
The Context is the *wiring* plane — engine-created, per record, empty again by the end of
a well-formed graph (every cell freed after its last read). Nothing here is
``@configurable``; a Context never appears in YAML.

The engine (``Stream`` — and ``FlowGraph``, which manages its env directly) creates one
...
_CURRENT: contextvars.ContextVar[Optional[Context]] = contextvars.ContextVar("recordstream_context", default=None)


def current() -> Optional[Context]:
    """The active per-record :class:`Context`, or ``None`` outside an engine/`activate` block."""
    return _CURRENT.get()
...
def require(op_name: str = "context op") -> Context:
    ...
    if ctx is None:
        raise RuntimeError(
            f"{op_name}: no active Context. Context ops need the per-record Context the engine "
            f"creates — run the pipeline through Stream/FlowGraph, or wrap a manual loop in "
            f"`with recordstream.context.activate(Context()):`."
        )
    return ctx
...
def activate(ctx: Context) -> Iterator[Context]:
    """Activate ``ctx`` as the current per-record Context for the enclosed block."""

...
* :func:`batch_boxes` — a :class:`~recordstream.Boxes` column as per-record
...
from recordstream.items import Boxes, Record, is_item, item_value

if TYPE_CHECKING:  # torch is imported lazily at call time — this is annotation-only
    from torch import Tensor

__all__ = ["batch_boxes", "batch_metadata", "batch_tensor", "batch_values", "multi_hot", "per_record_predictions"]

#: The per-box PARALLEL ARRAY fields of a :class:`~recordstream.Boxes`, in the order a
...
_BOX_FIELDS = ("boxes", "labels", "scores")
...
def batch_boxes(batch: Record, key: str) -> List[Dict[str, Any]]:
    """A collated :class:`~recordstream.Boxes` column back into PER-RECORD dicts.

    The collate cannot stack a box set — every record has its own N — so it leaves each
    ...
    **Framework-free, deliberately.** The values are handed back EXACTLY as the record carried
    ...
    Args:
        batch: A batched record — from EITHER collate (``"record"`` leaves one batched
            :class:`~recordstream.Boxes` with per-record columns; ``"list"`` leaves a list of
            per-record ``Boxes``; both are read here).
        key: The record key holding the collated :class:`~recordstream.Boxes`.
    ...
    Raises:
        TypeError: when ``key`` does not hold a :class:`~recordstream.Boxes`.
        ValueError: when the item is not COLLATED (its ``boxes`` is not a per-record list) —
            passing a single record's ``Boxes`` here is the mistake the message names.

    Example::

        targets = batch_boxes(batch, "target")     # [{"boxes": [N0, 4], "labels": [N0]}, …]
        targets = [{k: v.to(device) for k, v in t.items()} for t in targets]   # a torch caller
    """
    item = batch[key]
    # The "list" collate leaves a LIST of per-record Boxes; the default leaves ONE batched
    # Boxes whose attrs are per-record lists. Both mean the same thing, so both read the same
    # — a consumer never branches on which collate ran.
    if isinstance(item, list):
        if not all(isinstance(entry, Boxes) for entry in item):
            raise TypeError(f"batch_boxes: {key!r} holds a list whose entries are not all Boxes.")
        return [
            {name: getattr(entry, name) for name in _BOX_FIELDS if getattr(entry, name, None) is not None}
            for entry in item
        ]
    if not isinstance(item, Boxes):
        raise TypeError(f"batch_boxes: {key!r} holds {type(item).__name__}, not a Boxes.")
    if not isinstance(item.boxes, list):
        raise ValueError(
            ...
            "Pass the batched record, not a single record's Boxes."
        )
    columns = {name: getattr(item, name) for name in _BOX_FIELDS if isinstance(getattr(item, name, None), list)}
    return [

...
// Generated by: aisland jenkins scaffold --project sampleflux
// Source template: aisland/services/jenkins.py (JenkinsService.scaffold_pipelines)
// To regenerate:  aisland jenkins scaffold --project sampleflux --force
// =========================================================================
pipeline {
    agent any
                ...
                stage('Black') {
                    steps {
                        script {
                            ...
                            def targets = sh(script: "for d in sampleflux tests examples; do if [ -d \"\$d\" ] && find \"\$d\" -name '*.py' | grep -q .; then printf \"%s \" \"\$d\"; fi; done || true", returnStdout: true).trim()
                            if (targets) {
                        ...
                        always {
                            script {
                                if (fileExists('black-checkstyle.xml')) {
                                    recordIssues(
                                        id: 'black-sampleflux',
                                        name: 'Black Formatting (Sampleflux)',
...
" """
                        ...
                        always {
                            script {
                                if (fileExists('isort-checkstyle.xml')) {
                                    recordIssues(
                                        id: 'isort-sampleflux',
                                        name: 'Isort Import Order (Sampleflux)',
                        ...
                        always {
                            script {
                                if (fileExists('flake8.txt') && readFile('flake8.txt').trim()) {
                                    recordIssues(
                                        id: 'flake8-sampleflux',
                                        name: 'Flake8 (Sampleflux)',
            ...
            steps {
                script {
                    if (fileExists('tests') && sh(script: "find tests -name '*.py' | grep -q .", returnStatus: true) == 0) {
                        sh "${VENV_BIN}/pytest tests --junitxml=test-report.xml --cov=sampleflux --cov-report=xml:coverage.xml --cov-report=term"
                    } else {
                ...
                always {
                    script {
                        ...
                        if (fileExists('coverage.xml')) {
                            recordCoverage(
                                id: 'coverage-sampleflux',
                                name: 'Sampleflux Coverage',
    ...
    post {
        always {
            echo 'Sampleflux Pipeline Complete.'
        }
        success {
            echo 'Sampleflux is healthy.'
        }
        failure {
            echo 'Sampleflux build failed. Please check linting or test failures.'
        }
    }
}

...
sampleflux dependency) source of both:
...
It is a library module (like :mod:`sampleflux.labels` / :mod:`sampleflux.projection`), **not**
``@configurable`` and not entry-pointed. The numpy ops in :mod:`sampleflux.ops.numpy`
(``WindowOp`` / ``SpectrumScalingOp`` / ``FourierOp``) and their torch mirrors in
:mod:`sampleflux.ops.torch` all reuse it — the torch ops take the numpy window coefficients

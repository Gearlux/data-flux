"""Key projection for SampleFlux sources — read only the record keys you need.
...
from sampleflux.items import Label, Record, is_item, item_data
...
def iter_key(source: Any, key: str) -> Iterator[Any]:
    ...
    A :class:`~sampleflux.items.Label` unwraps to its ``.value`` (the class id / name); any
    other registered item unwraps to its payload via :func:`~sampleflux.items.item_data`; a

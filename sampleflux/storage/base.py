...
def require_record_format(found: Any, where: str) -> None:
    ...
    ``sampleflux_format`` tag is missing or different was written before the plain-dict
    record model and must be re-generated with a current sink.
    """
    if found == TYPED_FORMAT:
        return
    detail = "no sampleflux_format tag" if found is None else f"sampleflux_format={found!r}"
    raise ValueError(
...
class DataSource(Protocol):
    """Minimum contract for a SampleFlux data source."""
...
class DataSink(Protocol):
    """Minimum contract for a SampleFlux data sink."""

...
A transform samples its random / configured parameters ONCE per record (:meth:`Transform.get_params`),
then walks the dict and, for each value whose type it handles, applies the registered
kernel (:mod:`sampleflux.dispatch`). Values it does not handle pass through untouched.
...
AS-IS; the engine invokes each op family natively (see ``sampleflux.core._apply_op``).
...
from sampleflux.dispatch import Kernel, dispatch, register_kernel
...
class Transform:
    ...
    type via ``@MyOp.kernel(ItemType)``. Override :meth:`get_params` to sample shared

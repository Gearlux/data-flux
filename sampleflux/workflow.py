...
``sampleflux run workflow.yaml`` runner, and — being plain ``@configurable``
...
Branches are held as INSTANCES and only ``run()`` when selected. Example::
    ...
    !class:sampleflux.workflow.Sequence
    steps:
      - !lazy:DownloadData
      - !class:sampleflux.workflow.Conditional
        condition: !class:sampleflux.workflow.PathExists { path: $MODEL_ROOT/model.ckpt }
...
The combinators inherit :class:`~sampleflux.runnable.TorchRunner` and
:class:`~sampleflux.runnable.ProgressReporting` so a workflow runs correctly on a

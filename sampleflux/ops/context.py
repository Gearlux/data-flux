"""Context ops — move data between the per-record :class:`~sampleflux.context.Context` and the stream.
...
these (``sampleflux.flow.to_ops``), executable by the ordinary ``Flux`` engine — and lifts
back (``from_ops``).

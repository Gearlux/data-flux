...
Pins the native typed transforms that let a ``TypedSample`` detection pipeline build its
...
from sampleflux import Image, Label, Mask, Regions, TypedSample
...
class TestCocoToTorchVisionDetection:
    def test_produces_target_regions(self) -> None:
        s = TypedSample({"objects": Label(_OBJECTS)}, roles={"objects": "aux"})
    ...
    def test_parity_with_legacy(self) -> None:
        typed = CocoToTorchVisionDetection(field="objects")(TypedSample({"objects": Label(_OBJECTS)}))
        legacy = CocoToTorchVisionDetectionOp()(Sample(input=None, target=_OBJECTS, metadata={})).target
        assert torch.equal(typed["target"].boxes, legacy["boxes"])
        assert torch.equal(typed["target"].labels, legacy["labels"])
    ...
    def test_parity_xyxy_and_label_offset(self) -> None:
        ...
        typed = CocoToTorchVisionDetection(field="objects", bbox_format="xyxy", label_offset=1)(
            TypedSample({"objects": Label(objects)})
        )
        legacy = CocoToTorchVisionDetectionOp(bbox_format="xyxy", label_offset=1)(
    ...
    def test_missing_field_raises(self) -> None:
        with pytest.raises(ValueError, match="field 'nope' not in sample"):
            CocoToTorchVisionDetection(field="nope")(TypedSample({"objects": Label(_OBJECTS)}))

    def test_empty_sample_raises(self) -> None:
        with pytest.raises(ValueError, match="sample is empty"):
            CocoToTorchVisionDetection()(TypedSample({}))
    ...
    def test_non_dict_source_raises(self) -> None:
        ...
        with pytest.raises(TypeError, match="objects mapping"):
            CocoToTorchVisionDetection(field="objects")(TypedSample({"objects": Label("not a dict")}))
...
class TestMasksToDetectionBoxes:
    def test_instance_mask_produces_target_regions(self) -> None:
        s = TypedSample({"mask": Mask(_instance_mask())}, roles={"mask": "aux"})
    ...
    def test_instance_parity_with_legacy(self) -> None:
        mask = _instance_mask()
        typed = MasksToDetectionBoxes(field="mask")(TypedSample({"mask": Mask(mask)}))
        legacy = MasksToDetectionBoxesOp()(Sample(input=None, target=mask, metadata={})).target
        assert torch.equal(typed["target"].boxes, legacy["boxes"])
        assert torch.equal(typed["target"].labels, legacy["labels"])
    ...
    def test_connected_components_parity(self) -> None:
        ...
        typed = MasksToDetectionBoxes(field="mask", connected=True, label=2)(TypedSample({"mask": Mask(binary)}))
        legacy = MasksToDetectionBoxesOp(connected=True, label=2)(Sample(input=None, target=binary, metadata={})).target
    ...
    def test_min_area_drops_small_instances(self) -> None:
        mask = _instance_mask()
        typed = MasksToDetectionBoxes(field="mask", min_area=10)(TypedSample({"mask": Mask(mask)}))
        legacy = MasksToDetectionBoxesOp(min_area=10)(Sample(input=None, target=mask, metadata={})).target
        assert torch.equal(typed["target"].boxes, legacy["boxes"])

    def test_empty_mask_yields_empty_tensors(self) -> None:
        out = MasksToDetectionBoxes(field="mask")(TypedSample({"mask": Mask(np.zeros((4, 4), dtype=np.uint8))}))
        assert out["target"].boxes.shape == (0, 4)
        assert out["target"].labels.shape == (0,)

    def test_default_picks_first_mask(self) -> None:
        s = TypedSample({"image": Image(np.zeros((2, 2, 3), dtype=np.uint8)), "seg": Mask(_instance_mask())})
        out = MasksToDetectionBoxes()(s)
        assert out["target"].boxes.shape == (3, 4)

    def test_new_output_field_keeps_source(self) -> None:
        s = TypedSample({"mask": Mask(_instance_mask())})
    ...
    def test_missing_field_raises(self) -> None:
        with pytest.raises(ValueError, match="field 'nope' not in sample"):
            MasksToDetectionBoxes(field="nope")(TypedSample({"mask": Mask(_instance_mask())}))

    def test_no_mask_or_array_field_raises(self) -> None:
        with pytest.raises(ValueError, match="no Mask or array-bearing field"):
            MasksToDetectionBoxes()(TypedSample({"lbl": Label("x")}))
...
def test_typed_collate_gathers_regions_as_list() -> None:
    a = CocoToTorchVisionDetection(field="objects")(TypedSample({"objects": Label(_OBJECTS)}))
    c = CocoToTorchVisionDetection(field="objects")(
        TypedSample({"objects": Label({"bbox": [[1.0, 2.0, 3.0, 4.0]], "category": [5]})})
    )
